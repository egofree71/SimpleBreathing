...
SimpleBreathing is a very simple Android breathing app built with **Godot 4.6.2**. The original implementation was written in **C#**; the `gdscript-port` branch is migrating the runtime code to **GDScript** while keeping the old C# files temporarily as reference.
...
│   └── Main.tscn
├── scripts/
│   ├── Main.cs                      # legacy C# reference during migration
│   ├── BreathingGauge.cs            # legacy C# reference during migration
│   ├── BreathingSettings.cs         # legacy C# reference during migration
│   ├── SettingsStorage.cs           # legacy C# reference during migration
│   ├── AppLocalization.cs           # legacy C# reference during migration
│   ├── main.gd
│   ├── breathing_gauge.gd
...
The old floppy-disk save icon system has been removed. The settings screen now auto-saves every change and no longer displays a save button.

## Project configuration

### `SimpleBreathing.csproj`

The original C# project file is still present temporarily as migration reference. On the `gdscript-port` branch, the active scene now uses `main.gd`, and `project.godot` no longer declares the project as a C# project.

The C# project files should be deleted only after the GDScript version has been validated on PC and Android.
...
The old save icon files have been removed from the `gdscript-port` branch because the settings screen now saves immediately and no longer needs a save button.
...
## Migration GDScript — neutralisation C#

La branche `gdscript-port` utilise maintenant les scripts GDScript comme implémentation active.
Les anciens fichiers C# ont été déplacés comme références texte dans `docs/csharp_reference/` afin que Godot ne tente plus de compiler le projet avec MSBuild.

Cette étape évite les erreurs de compilation C# pendant la migration tout en gardant le code original disponible pour comparaison.
